...
↓ psr4
↓ psr0
↓ helper // 直接加载文件，常用于helper文件
. 从上到下是TP5的优先顺序

@ 命名空间别名
* PHP的类别名
. Class Real { ... } // origin
. class_alias('Real', 'Alia', false); // 参数三是是否调用autoload
. new Alia(); // we can use alias to instantiate object
* 用此机制实现了命名空间别名 @this->autoload
* `class_exists`和`class_alias`默认都会试图调用`autoload`方法

@ `composer`自动加载
* 使用`composer`下载第三方库时，会自动生成以上各规范的自动加载文件
. 把各文件中的内容分别加载到本实例中
* `composer`自己也包含一个自动加载类，这里没有使用它
